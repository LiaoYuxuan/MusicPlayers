...
//    }
    ...
    protected void bindData(ViewHolder holder, int position, final Song data) {
        ...
        if (data.equals(playList.getPlayData())) {
            holder.setVisibility(R.id.music_isplay,0);
            holder.setTextColorRes(R.id.tv_position, R.color.white);
            holder.setTextColorRes(R.id.tv_title, R.color.main_color);
        } else {
            holder.setVisibility(R.id.music_isplay,8);
            holder.setTextColorRes(R.id.tv_position, R.color.text);
